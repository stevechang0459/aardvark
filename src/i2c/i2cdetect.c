...
//                         int first, int last)
int scan_i2c_bus(Aardvark handle)
{
	int i, j;
	int cmd;
	int res;
	...
	for (i = 0; i < 128; i += 16) {
		...
		for (j = 0; j < 16; j++) {
			...
			default: /* MODE_QUICK */
				...
				res = aa_i2c_write_ext(handle, i + j, AA_I2C_NO_FLAGS, 0, NULL, NULL);
				break;
			case MODE_READ:
				...
				u8 data;
				u16 num_read;
				res = aa_i2c_read_ext(handle, i + j, AA_I2C_NO_FLAGS, 1, &data, &num_read);
				if (res == 0 && num_read != 1)
					res = -1;
				break;
			}
